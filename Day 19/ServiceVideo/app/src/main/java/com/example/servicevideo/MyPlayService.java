        ...
        MediaPlayer.OnErrorListener, MediaPlayer.OnSeekCompleteListener, MediaPlayer.OnInfoListener, MediaPlayer.OnBufferingUpdateListener
{
    ...
    // sets all the listener interfaces
    @Override
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // if music is not playing, set the music link and prepare to start the music
    ...
    public void onDestroy() {
        ...
        // stops the music and releases all resources for the media player
        if (mediaPlayer != null)
        {
            if (mediaPlayer.isPlaying())
            {
    ...
    public void onCompletion(MediaPlayer mp) {
        // stops the media player and the service, calls onDestroy()
        if (mp.isPlaying())
        {
            mp.stop();
        }
    ...
    public void onPrepared(MediaPlayer mp) {
        // start the media player, called by .prepareAsync()
        if (!mp.isPlaying())
        {
            mp.start();
        }
